...
public interface ChapterDao {
  ...
  @SqlUpdate(
      "INSERT INTO chapter (uuid, name) "
          + "VALUES (:uuid, :name)"
          + "ON CONFLICT (name,sorrority_uuid) NO NOTHING")
  void insert(@BindBean ChapterRow chapterRow);

  @SqlQuery(
      "SELECT EXISTS (SELECT 1 FROM chapter WHERE (name = :name)AND(sorrority_uuid =: sorrorityUuid))")
  boolean exists(String name,UUID sorrorityUuid);

  @SqlQuery("SELECT * FROM chapter WHERE uuid = :uuid")
  Optional<ChapterRow> findBy(UUID uuid);

  @SqlUpdate("DELETE FROM chapter WHERE uuid = :uuid")
  void deleteChapter(UUID uuid);

  @SqlQuery("SELECT * FROM chapter")
